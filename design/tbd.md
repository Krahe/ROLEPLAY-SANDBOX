...
15. **Chaos severity parameterization** — Krahe call 2026-06-11: post-playtest. Spec §8 says the OVERCHARGE chaos roll scales with severity `(capacitor − profile.max) × 10`; code currently calls `rollChaosFizzle(state)` flat (same distribution for barely-overcharged and reckless fires). The fidelity cost scales with overshoot but the spectacle cost doesn't. Rewrite the chaos roll to take a severity input derived from actual overshoot; consider region-weighting (mild severity → atmospheric region, high severity → energetic/catastrophic regions).
