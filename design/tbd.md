...
15. **Chaos system pass** (expanded 2026-06-11 after full-table review) — Krahe call: post-playtest. The 20 entries are good (varied in consequence-space, several cut both ways / are tactically exploitable, persistent-state effects); the *system around them* is tamer than the table deserves. Three components:
    - **Severity scaling**: spec §8 says the OVERCHARGE chaos roll scales with `(capacitor − profile.max) × 10`; code calls `rollChaosFizzle(state)` flat. The entries already carry `severity` tags ("harmless"/"comedic"/"energetic") that nothing consumes — wire them: mild overshoot weights toward harmless/comedic, deep overshoot toward energetic/catastrophic.
