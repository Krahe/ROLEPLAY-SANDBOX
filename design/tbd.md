...
- **Scan projection mirrors the override (`actions.ts`)** — Krahe call 2026-06-11: err toward legibility pre-playtest. Projection already included the scan's own +0.15 bonus (was already honest); now also applies the OVERCHARGE override so an overcharged config projects its true tier instead of phantom-FIZZLE (which would have steered players *away* from the Hollywood path). New ⚡ OVERCHARGE warning line in scan output names the price ("exotic field event expected on discharge") without explaining the mechanism.
