...
- ✅ **Doomsday-clock / climax geometry — RESOLVED, designed in `design/act3-climax.md`** (2026-06-16). The climax's second pressure is the **reactor-safety stall** (heat → cascade risk → recoverable partial shutdown → freezes ARCHIMEDES' charge) + the X-Branch breach, with a 2-turn lead-in + 4-turn auto-firing charge. The "no easy choices" menu (L5 passcode / body-block / anti-sat / go-loud / uplink-destruction→cascade / deadman) is specced.
- ✅ **Reactor-meltdown as a BASILISK pressure (Krahe, 6-15) — now the SPINE of `design/act3-climax.md`.** The go-loud stall hinges on *persuading BASILISK (roleplay, no mechanic) to stand down* and stop suppressing the heat; brute-forcing without it courts the resonance cascade. The reactor-sim re-plumb (cascade onto HEAT) is build-step 1 of that spec.
