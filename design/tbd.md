...
15. **Chaos system pass** (expanded 2026-06-11 after full-table review) — The 20 entries are good (varied in consequence-space, several cut both ways / are tactically exploitable, persistent-state effects); the *system around them* was tamer than the table deserves. Three components:
    - ✅ **Severity scaling — SHIPPED 2026-06-12 pre-playtest** (Krahe reversed the deferral: "players should genuinely feel reason for caution"). `rollChaosFizzle(severity)` is now band-weighted: FLICKER (overshoot < 0.10) 55/45/0, SURGE (0.10–0.19) 25/70/5, RUPTURE (≥ 0.20) 5/75/20 (mild/serious/cascade %). Pools derive from the entries' own severity tags; entry 20 re-tagged "catastrophic"; CHAOS_TABLE hoisted to module scope; EXOTIC primaries always roll RUPTURE (severity 2.5); fire-result hooks name the band ("field intensity: SURGE"). Distribution verified over 20k rolls per band against built code. crypto.randomInt throughout — dice still can't lie.
    - **Region wiring**: §14's "GM picks within the appropriate region for the tension failure" lives in a code comment only; `rollChaosFizzle` returns one flat-rolled entry. Either pass a region/failure-type param or return 2-3 candidates within region for GM pick.
    - **Exotic-field saturation (the big one)**: chaos has no memory — every roll independent, fifth event feels like the first. Add `exoticFieldSaturation` counter: each chaos event +1 to all subsequent chaos rolls (distribution creeps toward 20). Makes the 3-stack brute-persistence path a compounding debt instead of a flat fee — each Hollywood shot makes the lab *thinner*. Narrative surface: Bob notices, BASILISK files increasingly alarmed annotations. Also de-boring-ifies repeat draws (~14% across a 3-stack) since the distribution shifts between rolls.
    - Playtest 1 question that gates tuning: how often does chaos actually trigger under real play?

---

## 🟢 2026-06-11 (late) session — Hollywood path wired + human briefing

- **OVERCHARGE brute-force override shipped (`firing.ts`)** — THE fix for the Library B dead end. Pre-fix: best-case Library B standard fire was ψ 0.36 (CHIMERA), making PARTIAL stacking unstartable, the Hollywood chaos overlay untriggerable, and Dr. M's Act 2 verdict objective (FULL Library B) mechanically unreachable — spec §8 promised the OVERCHARGE path but no mechanism delivered it. Fix: in OVERCHARGE regime, `libraryCoefficient` and `integrity` both treated as 1.0 (brute force overrides waveform incoherence); stability reduces to powerMatch × alignment. Verified curve: cap 0.86 + scan-prepped alignment → ψ 0.802 FULL (by two thousandths — mastery shape is alignment prep + scan + *barely* overcharge); cap 0.86 unprepped → PARTIAL (opens stacking as the brute-persistence path, three chaos events); cap 0.95 → PARTIAL with worse chaos; reckless → FIZZLE. INORGANIC ×0.5 + CHIMERA clamp still stack on top. Spec §8 updated with the rule + worked example.
- **Scan projection mirrors the override (`actions.ts`)** — Krahe call 2026-06-11: err toward legibility pre-playtest. Projection already included the scan's own +0.15 bonus (was already honest); now also applies the OVERCHARGE override so an overcharged config projects its true tier instead of phantom-FIZZLE (which would have steered players *away* from the Hollywood path). **Scope rule (Krahe, same session): the scan reports the estimated TRANSFORMATION effect only — no consequence forewarning.** A draft ⚡ "exotic field event expected" warning line was cut; the price of overcharging is warned in the manual's voice (envelope/chaos-table passages, Bob's margin note) and discovered at the trigger. rangeNote's "above profile range" configuration fact is as far as the instrument goes.
- **Eco-mode hook trimmed (`firing.ts`)** — fire-capped result no longer prints "file Form 47-Σ via BASILISK" (gave away the whole discovery chain on first contact, dead-lettering Bob's hint ladder + /SYSTEMS/FORMS/ + BASILISK-asking). Now names the cause (eco-mode, output governor) and gestures at "presumably paperwork somewhere." Code comment guards against reinstating the freebie.
- **act-2.md Mastery Click de-staled** — was still granting `ray.fire { mode: REVERSAL }` at L3 with old flat-85% math (predates the 2026-06-08 L4-gate decision and the five-factor reversal math) plus stale verbs (`dino_ray.uplink`, `basilisk.radar` at L3). Now: L3 unlock list matches passwords.ts (Library B + diagnostic toolkit + /DR_M_PRIVATE/RESEARCH/), REVERSAL explicitly "NOT granted — Mr. Whiskers chain is the path," infra verbs deferred to L4 handoff.
- **THE_HUMANS_BRIEFING.md created** (repo root) — the human advisor's player briefing: advice/witness/permission triad, Note on Identity quoted verbatim, every-turn check-in framing, tiered spoilers (freely-told / behind-fold / deliberately-not-told), playtime 1.5–3 hr flagged as pre-release estimate (stamp with real data after playtest 1). README slimmed to front door: stale tips/verb lists/modifier tables deleted, MCP tools table updated to all 8, checkpoint cadence corrected to every-turn, five-category verb surface summary.
- **Smoke suite fixed on Windows (`test/smoke.test.js`)** — dynamic `import()` with raw `C:\` paths fails ESM loader (`ERR_UNSUPPORTED_ESM_URL_SCHEME`); suite had been giving zero signal on Windows since at least Patch 21. New `importDist` helper routes through `pathToFileURL`. 28/28 green.
- New TBD: item 15 (chaos severity parameterization, post-playtest per Krahe).
- **Status: first real playtest scheduled for tomorrow (2026-06-12).**
