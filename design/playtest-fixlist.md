...
**REMAINING:** **Path 3 (delicate)** — wire a real deadman-DISARM + decide whether neutralizing Dr. M + disarming = a genuine prevention win (today neutralizing her alone arms the auto-fire). **Discoverability** — advertise `shutdown` (L5) to the PLAYER in the access dumps / ARCHIMEDES readout (the body-block is now GM-facing and already in the GM override docs). **Optional Path 1 polish** — a mechanical `handleBattle` → ARCHIMEDES_STOPPED coupling instead of pure GM narration.
