...
- **Path 5** — `infra.archimedes.block_uplink {who}` (L4) → `setUplinkBlocker` (auto-detects if the blocker's already a dino → harmless dissipate/win vs human → cascade). New verb routed to the L4 saboteur tier in `getInfraControlKey`.
...
**REMAINING:** **Path 3 (delicate)** — wire a real deadman-DISARM + decide whether neutralizing Dr. M + disarming = a genuine prevention win (today neutralizing her alone arms the auto-fire). **Discoverability** — advertise `shutdown` (L5) + `block_uplink` (L4) in the access dumps / GM Act-3 context so they're findable. **Optional Path 1 polish** — a mechanical `handleBattle` → ARCHIMEDES_STOPPED coupling instead of pure GM narration.
