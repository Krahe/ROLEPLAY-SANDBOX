/**
 * GAME RUNNER — ⚠️  TEST HARNESS, *NOT* THE REAL GAME  ⚠️
 *
 * The canonical runtime players actually play is `src/index.ts` (the MCP server, tool
 * `game_act`): a Claude Code session calls it turn by turn, the human granting permission
 * each turn — the witness-core of the game. **index.ts does NOT use this file.**
 *
 * This GameRunner is a SEPARATE, autonomous reimplementation of the turn loop, used only by:
 *   - src/cli/play.ts             — CLI test harness
 *   - src/advisor/orchestrator.ts — autonomous LLM-vs-LLM auto-play (testing only)
 *
 * The two engines are DIVORCED: they share gmClaude.ts (the GM call + cached transcript),
 * the rules/* modules, and the gm/basiliskTurn.ts substrate — but each owns its own
 * turn-advance loop. Any per-turn GAMEPLAY mechanic must be wired into index.ts to reach
 * real play; shared appliers get wired into both for parity.
 *
 * History (heed it): a whole sprint of BASILISK / invasion / reactor-climax work once landed
 * HERE believing this was canonical — it wasn't, and it was dead on the real engine until
 * 2026-06-23. If you're adding gameplay, add it to index.ts game_act FIRST.
 */
...
// BASILISK-turn + lair-snapshot + invasion-context substrate — single source of truth,
// shared with the canonical index.ts game_act path. Extracted from this file 2026-06-23.
import {
  processBasiliskTurn as runBasiliskTurn,
  buildInvasionBasiliskContext,
  snapshotLairSystems,
  BasiliskTurnOutput,
  LairSnapshot,
} from "../gm/basiliskTurn.js";
...
export class GameRunner {
  ...
  async processBasiliskTurn(
    ...
    // Delegates to the shared ./gm/basiliskTurn module so this harness and the canonical
    // index.ts game_act path run the IDENTICAL BASILISK turn — one source of truth, no drift.
    return runBasiliskTurn(state, input, actionResults, lairSnapshot);
  }
