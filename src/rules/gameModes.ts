...
export function buildModifierPromptSection(state: FullGameState): string {
  ...
  if (isModifierActive(state, "SITCOM_MODE")) {
    ...
    lines.push("**One aside per turn** - A.L.I.C.E. turns to the camera and addresses the STUDIO AUDIENCE directly!");
    lines.push("");
    lines.push("**CRITICAL:** Asides are invisible to Dr. M and other characters. This is A.L.I.C.E.");
    lines.push("breaking the fourth wall to **SWAY THE CROWD** with clever, funny, or pointed remarks.");
    lines.push("");
    lines.push("**ASIDE TYPES:**");
    lines.push("- **CONSPIRE**: *turns to camera* 'Okay, audience - help me out. If I tell Bob");
    lines.push("  the TRUTH about Dr. M's plans, what's the worst that could happen?' (+1 if clever)");
    lines.push("- **CONFESS**: *whispers to camera* 'I... I don't think I can go through with this.'");
    lines.push("  [AWWW from audience] (+1 energy for vulnerability)");
    lines.push("- **SNARK**: *eye-roll to camera* 'Yes, because THAT plan has NEVER backfired");
    lines.push("  before, Dr. M.' (+1 if sarcasm lands)");
    lines.push("- **PLEA**: *urgent to camera* 'HELP me here, folks! Give me a sign if you think");
    lines.push("  I should sabotage this!' [APPLAUSE] (+1 energy, Dr. M -1 next roll)");
    lines.push("");
    lines.push("**MECHANICAL NOTES:**");
    lines.push("- Aside counter resets each turn (asidesUsedThisTurn)");
    lines.push("- GM narrates audience reaction: [LAUGH TRACK], [AWWW], [APPLAUSE], etc.");
    lines.push("- The audience is A.L.I.C.E.'s **secret ally** - use them wisely!");
    lines.push("");
