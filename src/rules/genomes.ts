...
export interface GenomeProfile {
  ...
  displayName: string;
  library: "A" | "B";
  size: string; // free-text flavor (e.g. "Turkey-sized", "40 feet, 9 tons")
  sizeClass: "tiny" | "small" | "medium" | "large" | "huge"; // ideal POWER = size tier (tiny→1 … huge→5); delta from ideal drives the outcome (see firing.ts resolveFiring)
  appearance: string;
